...
class CountryPickerUtils {
  static Country getCountryByIso3Code(String iso3Code) {
    try {
      return countries.firstWhere(
            (country) => country.code.toLowerCase() == iso3Code.toLowerCase(),
      );
    } catch (error) {
  ...
  static Country getCountryByIsoCode(String isoCode) {
    try {
      return countries.firstWhere(
            (country) => country.flag.toLowerCase() == isoCode.toLowerCase(),
      );
    } catch (error) {
  ...
  static Widget getDefaultFlagImage(Country country) {
    return Image.asset(
      CountryPickerUtils.getFlagImageAssetPath(country.flag),
      height: 20.0,
      width: 30.0,
      fit: BoxFit.fill,
  ...
  static Country getCountryByPhoneCode(String phoneCode) {
    try {
      return countries.firstWhere(
            (country) => country.dialCode.toLowerCase() == phoneCode.toLowerCase(),
      );
    } catch (error) {
      throw Exception(
